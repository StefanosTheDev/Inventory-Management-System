from .AdminModel import UserModel
...
from extension import db  # Importing db instance from app.py
...
class AdminService:
    ...
    def create_user(username, password, email, role):
        try:
            validated_username = AdminService.check_username(username)
            validated_pass = AdminService.check_password(password)
            validated_email = AdminService.check_email_exist(email)
    ...
    def update_user(id, updatedUsername, updatedPass, updatedEmail, updatedRole):
        ...
        # Check and update username if changed
            if updatedUsername and updatedUsername != user.username:
                validated_username = AdminService.check_username(updatedUsername)
                user.username = validated_username
        
        # Check and update password if changed
            if updatedPass:
                validated_pass = AdminService.check_password(updatedPass)
        ...
        # Check and update email if changed
            if updatedEmail and updatedEmail != user.email:
                validated_email = AdminService.check_email_exist(updatedEmail)
    ...
    def Id_Req_Validation(request_id):
        ...
        parsedId = AdminService.removeAllSpaces(request_id)
        try:
            return int(parsedId)
        except ValueError:
